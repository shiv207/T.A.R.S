...
def listen_for_wake_word(audio):
    ...
    with open("wav_files/wake_detect.wav", "wb") as f:
        f.write(audio.get_wav_data())
    result = tiny_model.transcribe('wav_files/wake_detect.wav')
...
def prompt_gpt(audio):
    global listening_for_wake_word
    try:
        with open("wav_files/prompt.wav", "wb") as f:
            f.write(audio.get_wav_data())
        result = base_model.transcribe('wav_files/prompt.wav')
        ...
        if len(prompt_text.strip()) == 0:
            ...
            print('User:', prompt_text[:50] + ('...' if len(prompt_text) > 50 else ''))
